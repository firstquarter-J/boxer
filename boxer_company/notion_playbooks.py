...
_LOCAL_PLAYBOOKS: tuple[dict[str, Any], ...] = (
    ...
    # Notion live 연동이 없어도 반복 운영 케이스는 자유답변에서 바로 참고할 수 있게 둔다.
    {
        "pageId": "local-playbook:mommybox-motion-disabled-auto-recording-timeout",
        "section": "마미박스 장애 대응",
        "kind": "runbook",
        "priority": "high",
        ...
        "previewLines": (
            "원인: v2.11.300부터 모션감지 사용안함 설정에서는 바코드 스캔 후 1시간이 지나면 자동으로 녹화를 시작해",
            "증상: 녹화시작을 누르지 않았거나 리모컨 작동 이력이 없어도 바코드 스캔 약 1시간 뒤 녹화가 시작될 수 있어",
            "확인: 장비 설정이 모션감지 사용안함인지, 바코드 스캔 시각과 녹화 시작 시각 차이가 약 1시간인지 확인해",
            "확인: 리모컨 작동 버튼 이력이나 시작 QR 스캔 이력이 없으면 이 스펙 동작으로 먼저 봐",
            "답변: 모션감지 사용안함 설정에서는 바코드 스캔 후 1시간이 지나면 자동 녹화가 시작돼. v2.11.300 버전부터 해당 방식으로 동작하고 있어",
            "주의: 파란 LED 모션 감지 대기나 종료 직후 재스캔 실패 케이스와는 다른 케이스야",
        ),
    },
    {
        "pageId": "local-playbook:mommybox-auto-recording-retry-gap",
        "section": "마미박스 장애 대응",
        "kind": "runbook",
        "priority": "high",
        ...
        "previewLines": (
            ...
            "판단: 첫 녹화와 세 번째 녹화는 정상인데 종료 직후 두 번째 재스캔만 실패하면 캡처보드 단선보다 직전 FFmpeg 정리 전 재진입으로 먼저 봐",
            "로그 패턴: C_STOPSESS 후 4~6초 안에 같은 바코드를 다시 스캔하면 모션 FFmpeg가 /dev/video0: Device or resource busy로 실패할 수 있어",
            "영향: 이때는 바코드 입력은 들어왔지만 캡처보드 장치가 아직 점유되어 녹화 준비 단계에서 실패하고 DB 영상 기록이 남지 않을 수 있어",
            "조치: 동일 증상 발생 시 바로 재녹화하지 말고 종료 후 약 10초 이상 기다린 뒤 다시 스캔하도록 안내해",
            "조치: 반복되면 해당 시각 로그에서 C_STOPSESS와 재스캔 간격, 첫 FFmpeg error, /dev/video0 busy 여부를 같이 확인해",
        ),
    },
    {
    ...
    },
    {
        ...
        "previewLines": (
            "정책: 핑크 바코드만 따로 허용하거나 막는 설정은 없어",
            "기능: 바코드 유효성 검사는 촬영 전에 이 바코드로 진행해도 되는지 먼저 확인하는 기능이야",
            "예시: 무료 바코드나 환불 처리된 바코드처럼 사용하면 안 되는 바코드는 이 기능이 켜져 있으면 막혀야 해",
            "운영 기준: 분만 병원에서 바코드 차단이 걸리게 하려면 바코드 유효성 검사가 켜져 있어야 해",
            "전제: 바코드 유효성 검사를 끄면 이런 확인 없이 바로 녹화가 진행돼",
            "제한: 특정 바코드만 예외로 두는 방식은 안 되고, 검사 전체를 켜거나 끄는 방식이야",
        ),
    },
)
