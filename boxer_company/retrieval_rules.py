...
def _build_company_retrieval_rules(evidence_payload: Any) -> str:
    ...
    if route == "device_audio_probe":
        return (
            ...
            "11) `판정`은 `정상`, `확인 필요`, `이상`, `점검 불가` 중 하나로만 적어.\n"
            "12) playbackDevices.deviceCount가 1개 이상이고 playbackTest.ok=true이며 summary.mixerMuted=false면 `판정: 정상`으로 적고, 안내에 `장비 자체는 정상으로 보여서 연결된 스피커 전원, 케이블, 입력 소스를 먼저 점검해`를 포함해.\n"
            ...
            "16) pactl이 없다는 이유만으로 실패 판단하지 말고, 필요하면 `pactl 미설치는 실패 원인으로 보지 않아` 정도로만 짧게 적어.\n"
            "17) `근거`는 짧은 관찰값만 `/`로 이어서 적어. 예: `재생 장치 2개 / Master 87% on / PCM 100% / speaker-test 성공`\n"
