...
# node-hid ABI 패치는 운영 장비 PM2를 재시작하므로 배포 후 별도 flag로 연다.
DEVICE_SCANNER_ABI_PATCH_ENABLED = (
    os.getenv("DEVICE_SCANNER_ABI_PATCH_ENABLED", "false").strip().lower()
    in {"1", "true", "yes", "on"}
)
DEVICE_SCANNER_ABI_PATCH_TIMEOUT_SEC = int(
    os.getenv("DEVICE_SCANNER_ABI_PATCH_TIMEOUT_SEC", "780")
)
DEVICE_SCANNER_ABI_PATCH_STATE_MAX_AGE_SEC = int(
    os.getenv("DEVICE_SCANNER_ABI_PATCH_STATE_MAX_AGE_SEC", "120")
)
...
# 대기 장비는 약 20분마다 상태를 보내므로 네트워크·수집 지연을 허용해 25분까지 신뢰한다.
DEVICE_HEALTH_MONITOR_REDIS_STANDBY_STALE_SEC = int(
    os.getenv("DEVICE_HEALTH_MONITOR_REDIS_STANDBY_STALE_SEC", "1500")
)
...
DEVICE_HEALTH_MONITOR_VOICE_GUIDE_COOLDOWN_SEC = int(
    os.getenv("DEVICE_HEALTH_MONITOR_VOICE_GUIDE_COOLDOWN_SEC", "10")
)
