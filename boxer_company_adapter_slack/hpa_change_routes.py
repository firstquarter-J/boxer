...
def _handle_hpa_change_request(
    ...
    if not _looks_like_hpa_change_request(context.question):
        return False
    ...
    if not config.enabled:
        context.reply("HPA 코드 변경 요청 접수 기능이 꺼져 있어", mention_user=False)
        return True
    ...
    if not allowed_user_ids or not allowed_channel_ids:
        context.reply(
            "HPA 코드 변경 요청의 허용 사용자 또는 채널 설정이 없어",
            mention_user=False,
        )
        return True
    if not context.user_id or context.user_id not in allowed_user_ids:
        context.reply("HPA 코드 변경 요청을 접수할 권한이 없어", mention_user=False)
        return True
    if context.channel_id not in allowed_channel_ids:
        context.reply("이 채널에서는 HPA 코드 변경 요청을 접수하지 않아", mention_user=False)
        return True
    ...
    if not event_ts or not thread_ts or not workspace_id or not channel_id:
        context.reply("Slack 요청 식별 정보가 부족해서 접수하지 못했어", mention_user=False)
        return True
    ...
    try:
        ...
        context.reply("HPA 코드 변경 작업 큐에 접수하지 못했어", mention_user=False)
        return True
    if not isinstance(result, HpaChangeSubmissionResult):
        context.logger.warning("Invalid HPA change submission result type")
        context.reply("HPA 코드 변경 작업 큐 응답을 확인하지 못했어", mention_user=False)
        return True
